...
import { getInstructor, saveInstructor } from '../../tools/api'
...
class Edit extends React.Component {
  ...
  componentDidMount() {
    if (isFinite(this.props.params.instructorId)) {
      getInstructor(this.props.params.instructorId, dados => {
        this.setState({
          dados,
          editando: true
        })
  ...
  salvar = () => {
    saveInstructor(this.props.params.instructorId, this.state.dados, (success) => {
      this.setState({
    ...
    return(
      <div>
        <Row>
          ...
          : null }
            <PageHeader>{this.state.editando ? 'Editar' : 'Adicionar'} Instrutor</PageHeader>
            ...
            <Row>
              ...
              <Col md={1}>
                <Link to={'/instructors'}><Button bsStyle="default">Cancelar</Button></Link>
              </Col>
            </Row>
          </Col>
        </Row>
