...
RSpec.describe Api::V1::CoursesController, type: :api do
  ...
  it "should get only instructor's courses" do
    user = User.create({:email => "instructor@knap.com.br", :name => "Patricia", :birthday => "1989-09-22", :profile => User::PROFILE_INSTRUCTOR, :password => "password", :password_confirmation => "password"})
    post "auth_user", {:email => "instructor@knap.com.br", :password => "password"}
    post_json = JSON.parse last_response.body
    header "Authorization", "Bearer #{post_json["auth_token"]}"
